...
float hann_window(int x, int N) {
    float res = 0.5 * (1.0 - cosf(2.0 * pi * x / (N - 1)));
    return res;
}
      ...
      for (n = 0; n < num_comps; n++) {
          for (r = 0; r < N; r++) {
              for (c = 0; c < N; c++) {
                  ...
                  sum += output_comps[n].buf[r * N + c];
              }
          }
      }

      // Do horizontal and vertical hann_window filtering (only process grey image)
      float mean = sum / (N * N);
      for (r = 0; r < N; ++r) {
          float w_r = hann_window(r, N);
          for (c = 0; c < N; ++c) {
              float w_c = hann_window(c, N);
              float val = output_comps->buf[r * N + c];
              val = (val - mean) * w_r * w_c;
              output_comps->buf[r * N + c] = val;
          }
      }
        ...
        { // "r" holds the true row index we are writing, since the image is
          // written upside down in BMP files.
            ...
            for (int c = 0; c < N; c++, dst++) {
                *dst = static_cast<io_byte>(std::clamp(src[c] + 128.0F + 0.5F, 0.0F, 255.0F)); // plus 128 for visualization
            }
