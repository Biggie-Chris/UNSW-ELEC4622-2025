...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        /* 5. Calculate RGB values over 3 set(rings) */
        float sum_R = 0.0f, sum_G = 0.0f, sum_B = 0.0f;
        int   cnt_R = 0, cnt_G = 0, cnt_B = 0;

        const int halfN = N >> 1;   // N/2
        const int N2 = N >> 1;   // N/2
        const int N4 = N >> 2;   // N/4
        const int N8 = N >> 3;   // N/8
        const int N16 = N >> 4;   // N/16
        ...
        for (int r = 0; r < N; ++r)
        {
            ...
            float* row = output_comp->buf + r * output_comp->stride;
            for (int c = 0; c < N; ++c)
            {
                int k2 = c - halfN;    
                int ak2 = std::abs(k2);
        ...
        if (cnt_R > 0) { R = sum_R / cnt_R; }
        if (cnt_G > 0) { G = sum_G / cnt_G; }
        if (cnt_B > 0) { B = sum_B / cnt_B; }
