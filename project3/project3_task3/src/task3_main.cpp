    ...
    int start_row, int start_col, int block_width, int block_height, int S)
{
    ...
    for (vec.y = -S; vec.y <= S; vec.y++)
        for (vec.x = -S; vec.x <= S; vec.x++)
        {
            ...
            for (r = 0; r < block_height; r++, rp += ref->stride, tp += tgt->stride)
                for (c = 0; c < block_width; c++)
                {
                    float diff = tp[c] - rp[c];
                    total += diff * diff;
                }

            mse = total / (block_width * block_height);
  ...
  main(int argc, char *argv[])
{
  ...
  uint8_t H = std::stoi(argv[4]);
  uint8_t S = std::stoi(argv[5]);
  int delta = std::stoi(argv[6]); // keypoints spacing
