...
/*****************************************************************************/
/*                  my_aligned_image_comp::derivative_gaussian               */
/*****************************************************************************/
...
// ! After comparsion, I think it's only micro optimization to change heap allocation to
// ! stack alloction in terms of speed as long as the default memory size is predefined so no ( 120-ish ms versus 110-ish ms)
// ! extra function calls for 'allocator'. (memeory allocations consume a lot of time).
// ! So in practice, heap allocations can be used if we need a large memeroy space and we should calculate 
// ! the estimated memory size to be allocated so that we can minimize the use of allocators.
/*****************************************************************************/
/*                  my_aligned_image_comp::derivative_gaussian               */
/*****************************************************************************/
//float* my_aligned_image_comp::derivative_gaussian_2(my_aligned_image_comp* in, float s, std::string mode) {
//    int s0 = static_cast<int>(s + 1.0F);
//    constexpr int FILTER_EXTENT = 18; // 6 * 3 = 18
//    constexpr int FILTER_TAPS = 37;
//
//    // Creat Gaussian PSF as an array on the heap
//    float filter_buf_g[FILTER_TAPS];
//    float* mirror_psf_g = filter_buf_g + FILTER_EXTENT; // `mirror_psf' points to the central tap in the filter
//    float gsum = 0.0F;
//    for (int i = -FILTER_EXTENT; i <= FILTER_EXTENT; ++i) {
//        mirror_psf_g[i] = 1.0F / (std::sqrtf(2 * pi) * s) * (std::expf(-(i * i) / (2 * s * s)));
...
//
//    // Creat Derivative Gaussian PSF as a array on the heap
//    float filter_buf_dg[FILTER_TAPS];
//    float* mirror_psf_dg = filter_buf_dg + FILTER_EXTENT; // `mirror_psf' points to the central tap in the filter
//    for (int i = -FILTER_EXTENT; i <= FILTER_EXTENT; ++i) {
//        mirror_psf_dg[i] = -i / (s * s) * (1.0F / (std::sqrtf(2 * pi) * s) * (std::expf(-(i * i) / (2 * s * s)))); // no need to normalize Derivateive Filter
//    }
